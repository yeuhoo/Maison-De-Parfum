...
const BottleIcon = () => (
  <svg
    ...
    aria-hidden="true"
  >
    <rect x="13" y="0" width="10" height="6" rx="1" fill="currentColor" opacity="0.35" />
...
function ShopPage({ initialProducts }: ShopContentProps) {
  const searchParams = useSearchParams();
  const [active, setActive] = useState<Category>("All");
  const [query, setQuery] = useState("");
  const { addToCart } = useCart();

  // Set active category from URL param (e.g. /shop?category=Floral)
  useEffect(() => {
    const cat = searchParams.get("category") as Category | null;
    if (cat && (["All", "Floral", "Woody", "Oriental", "Fresh"] as string[]).includes(cat)) {
      setActive(cat);
    }
  }, [searchParams]);
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-baseline gap-1.5">
                        ...
                        <span className="text-[11px]" style={{ color: "#7C6D5A" }}>
                          / 50ml
                        </span>
                      </div>
                      <button
      ...
      <section className="py-20 bg-background border-t border-(--muted-sand)">
        <div className="max-w-2xl mx-auto px-6 lg:px-8 text-center">
          ...
          >
            ...
            <p className="leading-relaxed" style={{ fontFamily: "var(--font-montserrat)", color: "#7C6D5A" }}>
